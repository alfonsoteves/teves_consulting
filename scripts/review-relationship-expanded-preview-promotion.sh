...
fail() {
  printf 'Relationship-expanded preview promotion review failed: %s\n' "$1" >&2
  exit 1
}
...
printf 'Building the relationship-expanded preview candidate locally...\n'
icp build "$BACKEND_CANISTER"
[[ -s "$GENERATED_CANDID" ]] || fail "The local build did not produce Candid"

printf 'Checking the additive response contract...\n'
...
if mops check-candid "$BASELINE_CANDID" "$GENERATED_CANDID" >/dev/null 2>&1; then
  fail "Candidate did not add the expected expandedMemories response field"
fi

baseline_methods="$(service_methods "$BASELINE_CANDID")"
candidate_methods="$(service_methods "$GENERATED_CANDID")"
[[ "$candidate_methods" == "$baseline_methods" ]] || fail "Candidate changed service methods"
...
require_line 'expandedMemories: vec MemoryPreview;' "$GENERATED_CANDID"
require_line 'previewMyContinuity: (queryText: text) -> (Result) query;' "$GENERATED_CANDID"
if grep -Fq 'expandedMemories: vec MemoryPreview;' "$BASELINE_CANDID"; then
  fail "Committed baseline already contains expandedMemories"
fi
...
if grep -Fq 'expandedMemories: vec MemoryPreview;' "$LIVE_CANDID"; then
  fail "Live backend already exposes expandedMemories"
fi
