...
fail() {
  printf 'Provider route preview promotion review failed: %s\n' "$1" >&2
  exit 1
}
...
printf 'Building the provider route preview candidate locally...\n'
icp build "$BACKEND_CANISTER"
[[ -s "$GENERATED_CANDID" ]] || fail "The local build did not produce Candid"

printf 'Checking the additive provider route preview contract...\n'
...
if mops check-candid "$BASELINE_CANDID" "$GENERATED_CANDID" >/dev/null 2>&1; then
  fail "Candidate did not add the expected provider route preview method"
fi

baseline_methods="$(service_methods "$BASELINE_CANDID")"
candidate_methods="$(service_methods "$GENERATED_CANDID")"
expected_candidate_methods="$(printf '%s\n%s\n' "$baseline_methods" 'previewAionProviderRoute' | sort -u)"
[[ "$candidate_methods" == "$expected_candidate_methods" ]] || fail "Candidate changed service methods beyond previewAionProviderRoute"

require_line 'previewAionProviderRoute:' "$GENERATED_CANDID"
require_line 'previewMyContinuity: (queryText: text) -> (Result) query;' "$GENERATED_CANDID"
if grep -Fq 'previewAionProviderRoute:' "$BASELINE_CANDID"; then
  fail "Committed baseline already contains previewAionProviderRoute"
fi
...
if grep -Fq 'previewAionProviderRoute:' "$LIVE_CANDID"; then
  fail "Live backend already exposes previewAionProviderRoute"
fi
